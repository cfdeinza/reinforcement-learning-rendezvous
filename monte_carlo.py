"""
This script evaluates trajectories using an existing model with a given set of initial conditions.
...
def evaluate(model, env, initial_state):
    ...
    pos_mask = pos_error < max_rd_error
    vel_mask = vel_error < max_vd_error
    att_mask = att_error < env.max_qd_error
    rot_mask = rot_error < env.max_wd_error
    all_mask = pos_mask * vel_mask * att_mask * rot_mask
    if np.any(all_mask):
        index = np.argmax(all_mask)
    else:
        three_mask = pos_mask * vel_mask * att_mask + pos_mask * vel_mask * rot_mask
        if np.any(three_mask):
            index = np.argmax(three_mask)
        else:
            two_mask = pos_mask * vel_mask
            if np.any(two_mask):
                index = np.argmax(two_mask)
            elif np.any(pos_mask):
                index = np.argmax(pos_mask)
            else:
                index = -1

    terminal_pos_error = pos_error[index:].mean()
    terminal_vel_error = vel_error[index:].mean()
    terminal_att_error = np.degrees(att_error[index:].mean())
    terminal_rot_error = np.degrees(rot_error[index:].mean())
...
if __name__ == "__main__":
    ...
    # arguments.model = r"C:\Users\charl\Downloads\rnn_model.zip"
    # arguments.input_file = os.path.join("verification", "initial_v2.csv")
    arguments.save = True
    main(arguments)
