...
"""
Verify that the angular displacement and rotation rate caused by a control torque has the expected effect.
Result: close enough. Small difference because the dynamic model assumes that all the torque is applied instanteaneously
at the beginning of the time step, instead of throughout the time step. The difference is small, so it does not matter.
"""
...
def main():
    ...
    torque = 0.2 / 2  # N.m
    wdot = torque / env.inertia[2, 2]
    print(f"M_z  = {torque} N.m")
    print(f"I_zz = {env.inertia[2, 2]} kg.m^2")
    print(f"wdot = {wdot} rad/s^2")
    tf = 32.5
    theta = 0.5 * wdot * tf ** 2
    w = wdot * tf
    print(f"Expected angular displacement after {tf} s = {theta} rad = {np.degrees(theta)} deg")
    print(f"Expected rotation rate after {tf} s        = {w} rad/s = {np.degrees(w)} deg/s")
    dw = torque / env.inertia[2, 2] * 0.5  # rad/s per step
    _ = env.reset()
    # rs = [env.rc]
    # vs = [env.vc]
    q = [env.qc]
    ...
    theta_ideal = np.degrees(0.5 * wdot * ts**2)
    w_ideal = np.degrees(wdot * ts)
    theta_exp = np.degrees(2 * np.arccos(q[:, 0]))
    w_exp = np.linalg.norm(w, axis=1) + np.linalg.norm(w[1])
    print(f"Terminal angular disp error:  {theta_exp[-1] - theta_ideal[-1]} deg")
    print(f"Terminal rotation rate error: {w_exp[-1] - w_ideal[-1]} deg/s")
    lw = 2
    fig, ax = plt.subplots(1, 2, figsize=(8,5))
    ax[0].plot(ts, theta_ideal, ".", label="Ideal angular displacement", lw=lw)
    ax[0].plot(ts, theta_exp, ".", label="Experimental angular displacement", lw=lw)
    ax[0].plot(ts, np.abs(theta_exp - theta_ideal), ".", label="Error", lw=lw)
    ax[1].plot(ts, w_ideal, ".", label="Ideal rotation rate", lw=lw)
    ax[1].plot(ts, w_exp, ".", label="Experimental rotation rate", lw=lw)
    ax[1].plot(ts, np.abs(w_exp - w_ideal), ".", label="Error", lw=lw)
    ax[0].set_ylabel("Angular displacement [deg/s]")
    ax[1].set_ylabel("Rotation rate [deg/s]")
    ax[0].set_ylim([0, 200])
    ax[1].set_ylim([0, 15])
    for axis in ax:
        axis.set_xlabel("Time [s]")
        axis.set_xlim([ts[0], ts[-1]])
        axis.grid()
        axis.legend()
    plt.show()
    plt.close()
